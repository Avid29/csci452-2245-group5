...
** "new_offsets.h". This can be compared to the current version
** ("include/offset.h") and copied over it if there are differences.
*/
...
void setheader( void ) {
	...
	hfile = fopen( "new_offsets.h", "w" );
	if( hfile == NULL ) {
		perror( "offsets.h" );
		exit( 1 );
	}

	fprintf( hfile, h_prefix );
}
...
int main( int argc, char *argv[] ) {
	...
	*/
	...
	process( "PCB", "stkpgs", offsetof(pcb_t,stkpgs) );
