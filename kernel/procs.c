...
void pcb_free( pcb_t *pcb ) {
	...
	if( pcb != NULL ) {
		...
		if( status != SUCCESS ) {
			sprinf( b256, "pcb_free(0x%08x) status %d", (uint32_t) pcb,
					status );
			PANIC( 0, b256 );
		}
	}
...
void pcb_zombify( register pcb_t *victim ) {
	...
	*/
	if( zchild != NULL && init_pcb->state == STATE_WAITING ) {
		...
		int status = pcb_queue_remove_this( waiting, init_pcb );
...
void pcb_dump( const char *msg, register pcb_t *pcb, bool_t all ) {
	...
	cio_printf( " %s",
			state_str[pcb->state] );
			p->state >= N_STATES ? "???" : state_str[pcb->state] );
	...
	cio_printf( " next %08x context %08x stack %08x pde %08x",
				(uint32_t) pcb->next, (uint32_t) pcb->context,
				(uint32_t) pcb->stack, (uint32_t) pcb->pagedir );

	// and the filler (just to be sure)
	cio_puts( " fill: " );
	for( int i = 0; i < sizeof(pcb->filler); ++i ) {
		cio_printf( "%02x", pcb->filler[i] );
	}
